...
class BuildTsunamiAE():
    ...
    #create new loss functions using G as a loss function and another weighted by L2 norm and assymetric to penalise underprediction more than overprediction

    def g_loss(self, recon, true):
        g_loss = 1 - (2*torch.sum(recon*true)/(torch.sum(recon**2)+torch.sum(true**2)))
    
    def wt_g_loss(self, recon, true):
        g_loss = 1 - (2*torch.sum(recon*true)/(torch.sum(recon**2)+torch.sum(true**2)))
        l2 = torch.sqrt(torch.sum(true**2)/len(true>0.1))
        wt_g_loss = g_loss*l2
        return wt_g_loss
    
    def custom_loss_asymmetric(self, recon, true):
        """
        Asymmetric loss function where underpredictions are penalized more than overpredictions.
        ...
        """
        alpha = 2.0  # Weight factor for underpredictions
        diff = recon - true
        underprediction_mask = (diff < 0).float()  # 1 if underprediction, else 0

        loss = torch.mean((alpha * underprediction_mask + 1) * (torch.abs(diff) ** 3)) + \
            0.5 * torch.mean((alpha * underprediction_mask + 1) * (torch.abs(diff) ** 2))

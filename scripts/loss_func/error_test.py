...
def run_experiment(MLDir,reg,reg_gaugeno,GaugeNo,windowthreshold,twindow,train_size,mask_size,test_size,batch_size,batch_size_on,
                   batch_size_deform,ts_dim,pts_dim,parts,z,h,channels_off,channels_on,channels_deform,task,loss_type,asym_alpha):
    ...
    AE.evaluateEDerror( #used to test direct model without pretraining or mc dropout estimates 
                   job = 'mc_dropout', #mc_dropout, no_dropout
                   data_in=t_array,
                   data_deformfull=dZ_array,
                   data_out=red_d_array,
                   batch_size = 100,

...
class BuildTsunamiAE():
    ...
    def mse_asym_sum(self, recon, true, alpha=2.0, beta=1.0):
        """
        ...
        alpha = 2.0 
        beta = 1.0
        loss = torch.sum(torch.where(recon < true, alpha * (true - recon) ** 2, beta * (recon - true) ** 2))
        return loss
    ...
    def mse_asymmetric_band(self, recon, true):
        ...
        loss_overpredict = torch.mean(torch.where(true > 5, beta * (recon - true) ** 2, (recon - true) ** 2))

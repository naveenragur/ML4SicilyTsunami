...
class EncoderDecoderError(nn.Module): #Model without pretraining
    def __init__(self,
                x, #input wave data
                y, #output depth data
                xy, #output size
        ...
        super(EncoderDecoderError, self).__init__()
        ...
        self.onshore_split_err_decoders = nn.ModuleList()
        for i in range(parts):
            err_decoder = nn.Sequential(
                nn.LazyLinear(self.split_size),
                nn.LeakyReLU(negative_slope=0.01, inplace=True),
                nn.Softplus(),
            )
            self.onshore_split_err_decoders.append(err_decoder)
    ...
    def forward(self, x, dz):
        ...
        for i in range(self.parts): #z[0]*parts to 600k/parts
            decoded_split = self.onshore_split_decoders[i](split_global_decoded[i])
            err_decoded_split = self.onshore_split_err_decoders[i](split_global_decoded[i])
            decoded_splits.append(decoded_split)
            err_decoded_splits.append(err_decoded_split)
        
        # Concatenate the decoded parts
        y = torch.cat(decoded_splits, dim=1) #600k/parts to 600k
        err_y = torch.cat(err_decoded_splits, dim=1) #600k/parts to 600k
        y = y[:, :self.xy] #crop to original size
        err_y = err_y[:, :self.xy]
...
class BuildTsunamiAE():
    ...
    def plot_save_loss(self):
            ...
            if self.job == 'couple' or self.job == 'couple_test' or self.job == 'coupled':
                plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/{self.job}_loss_off{self.channels_off}_on{self.channels_on}_{self.train_size}.png')
                ex.add_artifact(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/{self.job}_loss_off{self.channels_off}_on{self.channels_on}_{self.train_size}.png')
            elif self.job == 'withdeform' or self.job == 'nodeform':
                plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/{self.job}_loss_off{self.channels_off}_on{self.channels_on}_{self.train_size}.png')
                ex.add_artifact(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/{self.job}_loss_off{self.channels_off}_on{self.channels_on}_{self.train_size}.png')
            elif self.job == 'witherror':
                plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/{self.job}_loss_off{self.channels_off}_on{self.channels_on}_{self.train_size}.png')
                ex.add_artifact(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/{self.job}_loss_off{self.channels_off}_on{self.channels_on}_{self.train_size}.png')
                
            else:
                plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/{self.job}_loss_ch_{self.channels}_{self.train_size}.png')   
                ex.add_artifact(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/{self.job}_loss_ch_{self.channels}_{self.train_size}.png')
            plt.clf()

    #test these loss functions to handle imbalance of events and underprediction at extreme tail and overprediction at the initial tail
    ...
    def g_loss(self, recon, true):
        """
        Calculate G-loss at each event level and return the mean G-loss across all events.
        
        Parameters:
        recon (tensor): Reconstructed predictions for n batch events.
        true (tensor): Numerical simulations for n batch events.
        
        Returns:
        tensor: Mean G-loss across all events.
        """
        # Ensure recon and true are of the same shape
        assert recon.shape == true.shape, "Shape mismatch between recon and true tensors"
        
        # Calculate G-loss for each event
        g_loss_per_event = 1 - (2 * torch.sum(recon * true, dim=1) / (torch.sum(recon**2, dim=1) + torch.sum(true**2, dim=1)))
        
        # Return the mean G-loss across all events
        return torch.sum(g_loss_per_event)
    
    def wt_g_loss(self, recon, true):
        """
        Calculate Weighted(l2norm) G-loss at each event level and return the mean Weighted G-loss across all events.
        ...
        Returns:
        tensor: Mean Weighted G-loss across all events.
        """
        # Ensure recon and true are of the same shape
        assert recon.shape == true.shape, "Shape mismatch between recon and true tensors"
        ...
        # Calculate G-loss for each event
        g_loss_per_event = 1 - (2 * torch.sum(recon * true, dim=1) / (torch.sum(recon**2, dim=1) + torch.sum(true**2, dim=1)))
        l2norm = torch.norm(true, p=2, dim=1)

        # Return the mean G-loss across all events
        return torch.sum(g_loss_per_event * l2norm)

    def mse_asym_sum(self, recon, true):
        """
        Asymmetric loss function where underpredictions are penalized more than overpredictions in mse.

        Parameters:
        recon (tensor): Reconstructed predictions.
        true (tensor): Numerical simulations.

        Returns:
        tensor: Computed loss.
        """
        alpha = self.asym_alpha 
        loss = torch.sum(torch.where(recon < true, alpha * (true - recon) ** 2,  (recon - true) ** 2))
        return loss

    def mse_norm(self, recon, true):
        """
        ...
        tensor: Computed loss.
        """
        #number of location per batch event with inundation (could be small like few thousand or large like 600k)
        nloc_per_batcheve = torch.sum((true > 0.1) | (recon > 0.1), dim=1) #batch x 1

        #sum of squared errors over all locations in each batch event
        sse = torch.sum((recon - true) ** 2, dim=1) #batch x 1

        #normalised mse over inundated locations only
        loss = torch.mean(sse / (nloc_per_batcheve + 1e-6) )
    ...
    def asym_mse_scalesq(self, recon, true):
        """
        Loss function where mse is weighted higher if underpredicting.
        ...
        tensor: Computed loss.
        """
        #set gamma based on depth of inundation as (1 + depth**2) to handle zero depth
        gamma = 1 + true ** 2
        loss = torch.mean(torch.where(recon < true, gamma * (true - recon) ** 2,  (recon - true) ** 2))
        return loss
    
    def asym_mse_scalecu(self, recon, true):
        """
        Loss function where mse is weighted higher if underpredicting.
    ...
    def mse_asym_wtl2norm(self, recon, true):
        """
        Asymmetric loss function where underpredictions are penalized more than overpredictions in mse and weighted by l2norm of the events.
        ...
        beta = 1.0
        l2norm = torch.norm(true, p=2, dim=1)
        mse_asym = torch.where(recon < true, alpha * (true - recon) ** 2, beta * (recon - true) ** 2)
        # Compute the mean of mse_asym along the locations dimension (dim=1)
        mse_asym_mean = torch.mean(mse_asym, dim=1)
        loss = torch.sum(mse_asym_mean * l2norm)
        return loss

    def weighted_hybrid_loss(self, recon, true, gamma=2, alpha=0.7, beta=1.5, log_cosh_weight=0.3):
        """
        Hybrid loss function with event-level weighting, asymmetric penalty, and smooth loss.

        Parameters:
        recon (tensor): Predicted values (batch_size, locations).
        true (tensor): Ground truth values (batch_size, locations).
        gamma (float): Controls weight emphasis on larger values at event level.
        alpha (float): Weight for weighted MSE loss.
        beta (float): Factor for penalizing underpredictions more than overpredictions.
        log_cosh_weight (float): Weight for log-cosh loss.

        Returns:
        tensor: Computed loss.
        """

        # Compute event-level weights: Average true value per event (batch-wise)
        event_weights = (true.mean(dim=1) + 1).pow(gamma)  # Shape: (batch_size,)

        # Expand weights to match (batch_size, locations)
        event_weights = event_weights.unsqueeze(1)  # Shape: (batch_size, 1)

        # Asymmetric MSE Loss: Penalize underpredictions more
        asym_mse = torch.where(recon < true, beta * (true - recon) ** 2, (recon - true) ** 2)

        # Log-Cosh Loss: Smooth loss for stability
        log_cosh = torch.log(torch.cosh(recon - true))

        # Apply event-level weights to loss components
        loss = alpha * torch.mean(event_weights * asym_mse) + (1 - alpha) * log_cosh_weight * torch.mean(event_weights * log_cosh)

        return loss
    
    def mse_asymmetric(self, recon, true):
        """
        Asymmetric loss function where underpredictions are penalized more than overpredictions in mse.
        ...
        """
        alpha = self.asym_alpha
        loss = torch.mean(torch.where(recon < true, alpha * (true - recon) ** 2, (recon - true) ** 2))
        return loss
    
    def mse_asymmetric_band(self, recon, true):
        """
        Asymmetric loss function where underpredictions are penalized more than overpredictions in mse.
        Depths below 2.5m are exaggeratedly penalized as they are always overpredicted with ghost inundation by default.
        ...
        alpha = 1 + true ** 2
        beta = 1 + true ** 3
        loss_underpredict = torch.mean(torch.where(recon < true, alpha * (true - recon) ** 2, (recon - true) ** 2))
        loss_overpredict = torch.mean(torch.where((recon < 2.5) & (recon > true + 0.5) , beta * (recon - true) ** 2, 0))
        return loss_overpredict + loss_underpredict

    def mse_asymmetric_sqnorm(self, recon, true):
        """
        Loss function where mse is weighted higher if underpredicting after norm by a max depth.
    ...
    def mse_asymmetric_pwnorm(self, recon, true):
        """
        Loss function where mse is weighted higher if underpredicting after norm by a max depth.
    ...
    def mse_asymmetric_error(self, recon, sqresidual, true):
        """
        Asymmetric loss function where underpredictions are penalized more than overpredictions in mse.
        ...
        """
        alpha = 1 + true ** 3
        true_sqresidual = (recon - true) ** 2
        loss_predict = torch.mean(torch.where(recon < true, alpha * (true - recon) ** 2, (recon - true) ** 2))
        loss_residual = torch.mean((sqresidual - true_sqresidual) ** 2)
        hybrid_loss = loss_predict + loss_residual
        return hybrid_loss
    
    def nlog_like_error(self, mu, var, true):
        """
        Asymmetric loss function where underpredictions are penalized more than overpredictions in mse.
        ...
        loss = torch.mean(0.5 * torch.log(2 * np.pi * sigma ** 2) + 0.5 * ((true - mu) ** 2 / sigma ** 2))
        
        Returns:
        tensor: Computed loss.
        """
        var = torch.clamp(var, min=1e-6)  # Ensure sigma is not too small
        loss1 = torch.mean(torch.log(var))
        loss2 = torch.mean((mu - true) ** 2 / var)
    ...
    def mc_dropout_inference(self,control_points,trained_model, wave_input, deform_input, num_samples=50):
        ...
        Returns:
            mean_prediction: Mean of multiple predictions.
            uncertainty: Standard deviation of predictions.
        """
        trained_model.train()  # Set model to training mode with dropout
        predictions = torch.stack([trained_model(wave_input, deform_input) for _ in range(num_samples)])  # Multiple forward passes  
        #dim of prediction is num_samples x batch_size x num_locations
        
        #save the predictions at some point locations
        locindices = get_idx_from_latlon(control_points)
        mean_prediction = predictions.mean(dim=0)
        sigma_prediction = predictions.std(dim=0)  # Standard deviation as uncertainty measure
        
        return mean_prediction, sigma_prediction,predictions[:, :, locindices]
    ...
    def pretrain(self,
        ...
        for epoch in range(self.nepochs):
            ...
            #     torch.save(self.model, f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_{self.job}_ch_{self.channels}_epoch_{epoch}_{self.train_size}.pt')
            ...
            if epoch == 0:
                ...
                min_loss = (val_loss + test_loss)/(len(val_loader)+len(test_loader))
                min_epoch = epoch
                torch.save(self.model, f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_{self.job}_ch_{self.channels}_minepoch_{self.train_size}.pt')
            ...
            if epoch - min_epoch > self.es_gap:
                ...
                # torch.save(self.model, f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_{self.job}_ch_{self.channels}_estop_{epoch}_{self.train_size}.pt')
        ...
        ex.add_artifact(filename=f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_{self.job}_ch_{self.channels}_minepoch_{self.train_size}.pt')

        #plot and save loss as png and npy
        self.plot_save_loss()
        np.save(f'{self.MLDir}/model/{self.reg}/{self.task}/out/train_loss_{self.job}_ch_{self.channels}_{self.train_size}.npy', self.train_epoch_losses)
        np.save(f'{self.MLDir}/model/{self.reg}/{self.task}/out/test_loss_{self.job}_ch_{self.channels}_{self.train_size}.npy', self.test_epoch_losses)
    ...
    def finetuneAE(self,
        ...
        if self.off_size != '9999' and self.deform_size != '9999':
            if self.couple_epochs[0] == None :
                self.offshore_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_offshore_ch_{self.channels_off}_minepoch_{self.off_size}.pt",map_location=self.device)
                self.deform_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_deformfull_ch_{self.channels_deform}_minepoch_{self.deform_size}.pt",map_location=self.device)
                self.onshore_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_onshoreparts_ch_{self.channels_on}_minepoch_{self.train_size}.pt",map_location=self.device)
            elif self.couple_epochs[0] != None :
                self.offshore_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_offshore_ch_{self.channels_off}_epoch_{self.couple_epochs[0]}_{self.off_size}.pt",map_location=self.device)
                self.deform_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_deformfull_ch_{self.channels_deform}_epoch_{self.couple_epochs[1]}_{self.deform_size}.pt",map_location=self.device)
                self.onshore_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_onshoreparts_ch_{self.channels_on}_epoch_{self.couple_epochs[2]}_{self.train_size}.pt",map_location=self.device)
        else:
            if self.couple_epochs[0] == None :
                self.offshore_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_offshore_ch_{self.channels_off}_minepoch_{self.train_size}.pt",map_location=self.device)
                self.deform_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_deformfull_ch_{self.channels_deform}_minepoch_{self.train_size}.pt",map_location=self.device)
                self.onshore_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_onshoreparts_ch_{self.channels_on}_minepoch_{self.train_size}.pt",map_location=self.device)
            elif self.couple_epochs[0] != None :
                self.offshore_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_offshore_ch_{self.channels_off}_epoch_{self.couple_epochs[0]}_{self.train_size}.pt",map_location=self.device)
                self.deform_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_deformfull_ch_{self.channels_deform}_epoch_{self.couple_epochs[1]}_{self.train_size}.pt",map_location=self.device)
                self.onshore_model = torch.load(f"{self.MLDir}/model/{self.reg}/{self.task}/plot/model_onshoreparts_ch_{self.channels_on}_epoch_{self.couple_epochs[2]}_{self.train_size}.pt",map_location=self.device)
    ...
    def fulltuneED(self,
        ...
        if loss_type == 'custom': 
            self.criterion = self.custom_loss_off
        elif loss_type == 'g_loss':
            self.criterion = self.g_loss
        elif loss_type == 'wt_g_loss':
            self.criterion = self.wt_g_loss
        elif loss_type == 'mse_asym':
            self.criterion = self.mse_asymmetric
        elif loss_type == 'mse_norm':
            self.criterion = self.mse_norm
        elif loss_type == 'mse_asym_sum':
            self.criterion = self.mse_asym_sum
        elif loss_type == 'mse_asym_scale':
            self.criterion = self.asym_mse_scale
        elif loss_type == 'mse_asym_scalesq':
            self.criterion = self.asym_mse_scalesq
        elif loss_type == 'mse_asym_scalecu':
            self.criterion = self.asym_mse_scalecu
        elif loss_type == 'mse_asym_wtl2norm':
            self.criterion = self.mse_asym_wtl2norm
        elif loss_type == 'MSEsum':
            self.criterion = nn.MSELoss(reduction='sum')
        elif loss_type == 'weighted_hybrid_loss':
            self.criterion = self.weighted_hybrid_loss
        elif loss_type == 'mse_asym_band':
            self.criterion = self.mse_asymmetric_band
        elif loss_type == 'mse_asym_sqnorm':
            self.criterion = self.mse_asymmetric_sqnorm
        elif loss_type == 'mse_asym_pwnorm':
    ...
    def fulltuneEDerror(self,
        ...
        # Initialize model,criteria	and optimizer
        if self.job == 'witherror':
            self.model = EncoderDecoderError(ninputs=self.ninputs,
                                        x=self.x_dim,
                                        y=self.y_dim,
                                        xy=self.xy,
                                        ch_list=self.channels_off,
        ...
        if self.job == 'witherror':
            ...
            self.criterion = self.nlog_like_error
        elif loss_type == None:
            print('using default loss function MSELoss')     
        ...
        for epoch in range(self.nepochs):
            ...
            for batch_idx,(batch_data_in,batch_data_deformfull,batch_data_out) in enumerate(zip(train_loader_in,train_loader_deformfull,train_loader_out)):
                ...
                batch_data_deformfull = batch_data_deformfull[0].to(self.device)
                batch_data_out = batch_data_out[0].to(self.device)
                recon_data,residual_data = self.model(batch_data_in,batch_data_deformfull)
                loss = self.criterion(recon_data,residual_data, batch_data_out)
                train_loss += loss.item()
                loss.backward()
                self.optimizer.step()
                self.scheduler.step()
            ...
            for batch_idx,(batch_data_in,batch_data_deformfull,batch_data_out)  in enumerate(zip(val_loader_in,val_loader_deformfull,val_loader_out)):
                ...
                batch_data_out = batch_data_out[0].to(self.device)
                recon_data,residual_data = self.model(batch_data_in,batch_data_deformfull)
                vloss = self.criterion(recon_data,residual_data, batch_data_out)
                val_loss += vloss.item()
            ...
            for batch_idx,(batch_data_in,batch_data_deformfull,batch_data_out)  in enumerate(zip(test_loader_in,test_loader_deformfull,test_loader_out)):
                ...
                batch_data_out = batch_data_out[0].to(self.device)
                recon_data,residual_data = self.model(batch_data_in,batch_data_deformfull)
                tloss = self.criterion(recon_data,residual_data, batch_data_out)
                test_loss += tloss.item()
    ...
    def evaluateAE(self,
        ...
        if epoch is None:
            model = torch.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_coupled_off{self.channels_off}_on{self.channels_on}_minepoch_{self.train_size}.pt',map_location=torch.device('cpu'))
        else:
            model = torch.load(epoch,map_location=torch.device('cpu'))
            # model = torch.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_couple_off{self.channels_off}_on{self.channels_on}_epoch_{epoch}_{self.train_size}.pt',map_location=torch.device('cpu')) 
        model.eval()
        ...
        np.save(f'{self.MLDir}/model/{self.reg}/{self.task}/out/pred_trainsize{self.train_size}_testsize{self.test_size}.npy', predic)
        ...
        eve_perf.to_csv(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_coupled_off{self.channels_off}_on{self.channels_on}_{self.train_size}_eve_perf_testsize{self.test_size}.csv')
        ...
        plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_coupled_off{self.channels_off}_on{self.channels_on}_{self.train_size}_maxdepth_testsize{self.test_size}.png')
        plt.clf()
        ex.add_artifact(filename=f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_coupled_off{self.channels_off}_on{self.channels_on}_{self.train_size}_maxdepth_testsize{self.test_size}.png')        
        ...
        plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_coupled_off{channels_off}_on{channels_on}_{self.train_size}_error_testsize{self.test_size}.png')
        plt.clf()
        ex.add_artifact(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_coupled_off{channels_off}_on{channels_on}_{self.train_size}_error_testsize{self.test_size}.png')
        ...
        plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_coupled_off{channels_off}_on{channels_on}_{self.train_size}_gfit_testsize{self.test_size}.png')
        plt.clf()
        ex.add_artifact(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_coupled_off{channels_off}_on{channels_on}_{self.train_size}_gfit_testsize{self.test_size}.png')
        #save table
        table.to_csv(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_coupled_off{self.channels_off}_on{self.channels_on}_{self.train_size}_compile_testsize{self.test_size}.csv')
        ...
        true_pred_er.to_csv(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_coupled_off{self.channels_off}_on{self.channels_on}_{self.train_size}_true_pred_er_testsize{self.test_size}.csv')
    ...
    def evaluateED(self,
        ...
        if epoch is None:
            ...
            model = torch.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_withdeform_off{self.channels_off}_on{self.channels_on}_minepoch_{self.train_size}.pt',map_location=torch.device('cpu'))
        else:
            ...
            # model = torch.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_withdeform_off{self.channels_off}_on{self.channels_on}_epoch_{epoch}_{self.train_size}.pt',map_location=torch.device('cpu')) 
        model.eval()
    ...
    def evaluateEDerror(self,
        ...
        if self.job == 'no_dropout': 
            #read the direct no pretrain model from file for testing which predicts mu and sigma
            if epoch is None:
                print('using direct error model without pretraining')
                model = torch.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_witherror_off{self.channels_off}_on{self.channels_on}_minepoch_{self.train_size}.pt',map_location=self.device)
            else:
                print('using direct error model without pretraining with filepath as:',epoch)
                model = torch.load(epoch,map_location=torch.device('cpu')) #hardcoded model name from epoch input
                # model = torch.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_withdeform_off{self.channels_off}_on{self.channels_on}_epoch_{epoch}_{self.train_size}.pt',map_location=self.device) 
            model.eval() #

        elif self.job == 'mc_dropout': # to check mc dropout 2025_02_28
            #read the direct no pretrain model from file for testing
            if epoch is None:
                print('using mc dropout model without pretraining')
                model = torch.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_withdeform_off{self.channels_off}_on{self.channels_on}_minepoch_{self.train_size}.pt',map_location=self.device)
            else:
                print('using mc dropout  model without pretraining with filepath as:',epoch)
                model = torch.load(epoch,map_location=self.device)
        ...
        test_loader_out = self.dataloader(data_out)
        predic = np.zeros(data_out.shape)
        residual = np.zeros(data_out.shape)
        mc_samples =[]
        ...
        with torch.no_grad():
            ...
            for batch_idx,(batch_data_in,batch_data_deformfull,batch_data_out) in enumerate(zip(test_loader_in,test_loader_deformfull,test_loader_out)):
                ...
                if self.job == 'no_dropout':
                    recon_data,residual_data = model(batch_data_in,batch_data_deformfull)
                elif self.job == 'mc_dropout':
                ...
                #append new batch of samples (sample_size,batch_size,xlocation)
                mc_samples.append(point_samples.cpu().numpy())

                #calculate loss with neutral mse
                loss = self.criterion(recon_data, batch_data_out)
                test_loss += loss.item()
            ...
            mc_samples = np.concatenate(mc_samples, axis=1)  # Now shape (sample_size, total_batches * batch_size, xlocation)
    ...
    @ex.capture   #for direct model evaluation(without pretraining or deformation)
    def evaluateEDSingle(self,
        ...
        if epoch is None:
            ...
            model = torch.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_nodeform_off{self.channels_off}_on{self.channels_on}_minepoch_{self.train_size}.pt',map_location=torch.device('cpu'))
        else:
        ...
        #save prediction as numpy array
        np.save(f'{self.MLDir}/model/{self.reg}/{self.task}/out/pred_trainsize{self.train_size}_testsize{self.test_size}_nodeform.npy', predic)
        ...
        #save eve_perf as csv
        eve_perf.to_csv(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_nodeform_off{self.channels_off}_on{self.channels_on}_{self.train_size}_eve_perf_testsize{self.test_size}.csv')
        ...
        plt.tight_layout()
        plt.suptitle(f"mseoverall: {mseoverall:.5f}, r2maxdepth: {r2maxdepth:.5f}, gfitoverall: {gfitoverall:.4f}, testsize: {self.test_size}", fontsize=16, y=0.02)
        plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_nodeform_off{self.channels_off}_on{self.channels_on}_{self.train_size}_maxdepth_testsize{self.test_size}.png')
        plt.clf()
        ex.add_artifact(filename=f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_nodeform_off{self.channels_off}_on{self.channels_on}_{self.train_size}_maxdepth_testsize{self.test_size}.png')        
        ...
        for i in range(len(locindices)):
            ...
            axins.set_ylabel('Predicted')
            axins.set_aspect('equal', adjustable='box')
        plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_nodeform_off{self.channels_off}_on{self.channels_on}_{self.train_size}_error_testsize{self.test_size}.png')
        plt.clf()
        ex.add_artifact(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_nodeform_off{self.channels_off}_on{self.channels_on}_{self.train_size}_error_testsize{self.test_size}.png')
        ...
        plt.text(4, 0.3, f'Event Count by {bin_counts}', color='red')
        plt.title(f'G vs. max_off for {self.reg}', fontsize=15)
        plt.savefig(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_nodeform_off{channels_off}_on{channels_on}_{self.train_size}_gfit_testsize{self.test_size}.png')
        plt.clf()
        ex.add_artifact(f'{self.MLDir}/model/{self.reg}/{self.task}/plot/model_nodeform_off{channels_off}_on{channels_on}_{self.train_size}_gfit_testsize{self.test_size}.png')
        #save table
        table.to_csv(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_nodeform_off{self.channels_off}_on{self.channels_on}_{self.train_size}_compile_testsize{self.test_size}.csv')
        ...
        #save true_pred_er as csv
        true_pred_er.to_csv(f'{self.MLDir}/model/{self.reg}/{self.task}/out/model_nodeform_off{self.channels_off}_on{self.channels_on}_{self.train_size}_true_pred_er_testsize{self.test_size}.csv')

    @ex.capture   #reads postprocessed prediction and saves evaluation to results folder for finetuned coupled model
    def evaluatePredic(self,
        ...
        predic = np.load(f'{self.MLDir}/model/{self.reg}/{self.task}/out/postprocessed_trainsize{self.train_size}_testsize{self.test_size}.npy')

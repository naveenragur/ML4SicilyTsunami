...
np.random.seed(0)
if reg == 'CT':
    filename ='grid3-catania'
elif reg == 'SR':
    filename ='grid3-siracusa'

#file path for offshore time series
TSpath = SimDir + '/{:s}/grid0_ts.nc'
ncpath = SimDir + '/{:s}/{:s}.nc' #contains max height and deformation 
...
for i, event in enumerate(event_list):  
    ...
    #read nc files
    nc_grids = xr.open_dataset(ncpath.format(event,filename)) #grid file of depth
    ...
    #deformation
    dZ = nc_grids.deformation.values
    dZ = xr.DataArray(dZ, dims=nc_grids.deformation.dims, coords=nc_grids.deformation.coords,name='z') #create xr dataset
    dZ.to_netcdf(dZpath.format(event,reg),mode='w') #save

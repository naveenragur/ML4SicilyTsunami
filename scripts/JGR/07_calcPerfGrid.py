#Description: Calculates the performance metric at the prediction grids 
# #Usage: python 07_calcPerfGrid.py  <region> <mode> <train/test> <mask size> modes are:Grid/testfile/misfit
import os
import sys
...
def calc_scores(true,pred,rate=None):
    pred[pred < 0] = 0 #sometime the model predicts negative values instead of zero #not in this version 
    #apply rate
    if rate is not None:
        # rate = rate[idx]
        pred = pred*rate
...
if mode == 'Grid':
    ...
    eve_dep = pd.read_csv(f'{MLDir}/model/{reg}/multifoldMC/out/model_direct_off[64, 128, 256]_on[16, 128, 128]_{train_size}_true_pred_er_combined.csv')
    ...
    true_d = np.load(f'{MLDir}/model/{reg}/multifoldMC/PTHA/true_d_53550.npy').astype(np.int32)
    pred_d = np.load(f'{MLDir}/model/{reg}/multifoldMC/PTHA/pred_d_{train_size}_direct.npy').astype(np.int32)
    ...
    #save the table
    # np.save(f'{MLDir}/model/{reg}/multifoldMC/PTHA/Grid_scores_{train_size}.npy',table)
    ...
    table.to_csv(f'{MLDir}/model/{reg}/multifoldMC/PTHA/_Grid_scores_{train_size}.txt',index=False)
    ...
    eve_perf.to_csv(f'{MLDir}/model/{reg}/multifoldMC/PTHA/_eve_perf_{train_size}_PSHalf.txt',index=False)
    ...
    table['lat'] = index_map['lat']
    table['lon'] = index_map['lon']
    table.to_csv(f'{MLDir}/model/{reg}/multifoldMC/PTHA/_Grid_scores_{train_size}_PSHalf.txt',index=False)
    ...
    np.save(f'{MLDir}/model/{reg}/multifoldMC/PTHA/_misfit_{train_size}.npy',err_test)
    ...
    np.save(f'{MLDir}/model/{reg}/multifoldMC/PTHA/_TPE_table_{train_size}.npy',table_test)

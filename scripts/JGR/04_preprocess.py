#Description: Preprocess the offshore and onshore data for training and testing
#Usage: python 04_preprocess.py <region> <eventset size> <train/test> <mask size>
import os
import sys
...
for i, event in enumerate(event_list):
    ...
    d_array[i, :, :] = d_land
    # dZ_array[i, :, :] = dZ_land
    dZ_array[i, :, :] = dZ #use original deformation for full region
    t_array[i, :, :] = t
...
if mode == 'train':
    ...
    plt.savefig(f'{MLDir}/data/processed/lat_lon_idx_{reg}_{size}.png',dpi=300)

    #save to processed folder
    np.save(f'{MLDir}/data/processed/lat_lon_idx_{reg}_{size}.npy',lat_lon_idx)
    #as txt
    np.savetxt(f'{MLDir}/data/processed/lat_lon_idx_{reg}_{size}.txt',lat_lon_idx,fmt='%.5f',delimiter=',')

    #load mask instead of calculating it from given set size
    del zero_mask
    del non_zero_mask

...
def process_map(eve_name,train_size,tmap_array,map_array, min_area=10, save_plots=False, save_output=False, output_file=None):
    ...
    if save_plots:
        #check if post process directory exists
        # postprocess/PS_manning003
        # postprocess/PS_4-8_manning003
        # postprocess/PS_manning003
        # postprocess/BS_4-8_manning003
        if not os.path.exists(f'{MLDir}/model/{reg}/postprocess'):
            os.makedirs(f'{MLDir}/model/{reg}/postprocess')
        foldername = eve_name.split('/')[0]
        if not os.path.exists(f'{MLDir}/model/{reg}/postprocess/{foldername}'):
            os.makedirs(f'{MLDir}/model/{reg}/postprocess/{foldername}')

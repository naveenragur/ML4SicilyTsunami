...
try:
    ...
    size = sys.argv[2] #eventset size for test
    mode = sys.argv[3] #train or test
    mask_size = sys.argv[4] #mask size for training
except:
...
train_size = mask_size #event set size for training

#load events data from simulation directory
# event_list = os.listdir(f'{SimDir}')

event_list = np.loadtxt(f'{MLDir}/data/events/shuffled_events_{mode}_{size}.txt', dtype='str') #size is the event set size for testing prepared in 02_prepare_data.py
...
red_dZ_array = np.memmap(f'{MLDir}/data/processed/dZflat_{reg}_{train_size}.dat',
                            mode='r+',
                            dtype=float,
                            shape=(n_eve, nflood_grids))
...
utils.evaluateAE(data_in, #training data offshore
            data_deform, #training data onshore

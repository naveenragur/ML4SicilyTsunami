...
# input data
data = red_dZ_array[:int(len(red_dZ_array)*split),:]
test_data = red_dZ_array[int(len(red_dZ_array)*split):,:]
...
#architecture parameters - onshore/deformation
channels_on = [64,64]

#train model of type onshore or offshore
job = 'deform'
utils.pretrainAE(job, #offshore  or onshore or deformation or couple
            data, #training data 
            test_data, #test data 

...
OUTPUT_DIR = Path("/mnt/beegfs/nragu/tsunami/ML4SicilyTsunami/risk/hazard/npy_inputs")
...
size =  sys.argv[1]
pred_d = np.load(f'/mnt/beegfs/nragu/tsunami/ML4SicilyTsunami/model/CT/multifoldMC/PTHA/sigma_minus_{size}_direct.npy')
...
np.save(OUTPUT_DIR / f'pred_d_dask_{size}_sigmaminus.npy', pred_d_dask_filtered)
...
#save the flat files
np.save(OUTPUT_DIR / f'event_id_pred_{size}_sigmaminus.npy', event_id_dask_filtered)
...
#save the flat files
np.save(OUTPUT_DIR / f'site_id_pred_{size}_sigmaminus.npy', site_id_dask_filtered)

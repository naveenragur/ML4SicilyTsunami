...
size =  sys.argv[1]
pred_d = np.load(f'/mnt/beegfs/nragu/tsunami/ML4SicilyTsunami/model/CT/multifoldMC/PTHA/pred_d_{size}_direct.npy')
...
np.save(OUTPUT_DIR / f'pred_d_dask_{size}_mean.npy', pred_d_dask_filtered)
...
#save the flat files
np.save(OUTPUT_DIR / f'event_id_pred_{size}_mean.npy', event_id_dask_filtered)
...
#save the flat files
np.save(OUTPUT_DIR / f'site_id_pred_{size}_mean.npy', site_id_dask_filtered)
